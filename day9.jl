...
mutable struct Program
    raw_memory::Array{BigInt,1} # memory
    memory::OffsetArray{BigInt,1} # memory indexed from 0
    
    in::Array{BigInt,1} # input queue
    out::Array{BigInt,1} # output queue
...
function main()
    open("input9.txt", "r") do inputfile
        srccode = [parse(BigInt, x) for x in split(strip(read(inputfile, String)), ",")]

        out = Array{BigInt,1}()
        program = Program(copy(srccode), [1], out)
        run_program!(program)
        println("PART 1:")
        print_output(out)

        out = Array{BigInt,1}()
        program = Program(copy(srccode), [2], out)
        run_program!(program)
        println("PART 2:")
        print_output(out)
    end
